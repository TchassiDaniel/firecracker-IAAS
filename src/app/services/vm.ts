...
export const vmService = {
  ...
  async deleteVM(id: string): Promise<void> {
    const response = await fetch(`${API_URL}/delete_vm/${id}`, {
      method: "DELETE",
      headers: defaultHeaders,
    });
...
export const getVMs = async (userId) => {
  try {
    const response = await fetch(`${API_URL}/get_user_vms/${userId}`);
    if (!response.ok) {
      throw new Error(`HTTP error! status: ${response.status}`);
    }
    const data = await response.json();
    console.log("Réponse API getVMs:", data); // Debug log
    return data.vms; // Retourne directement le tableau vms
  } catch (error) {
    console.error("Erreur getVMs:", error);
    throw error;
  }
};

export const deleteVM = async (vmId: number): Promise<void> => {
  const response = await fetch(`http://localhost:8001/delete_vm/${vmId}`, {
    method: "DELETE",
    headers: {
      "Content-Type": "application/json",
    },
...
export const createVM = async (request: CreateVMRequest) => {
  ...
  const response = await fetch("http://localhost:8001/create_vm", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
